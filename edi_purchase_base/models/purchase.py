...
class PurchaseOrder(models.Model):
    ...
    def _consolidate_products(self):
        ...
        if not self.order_line:
            raise ValidationError(self.env._("No lines in this order %s!", self.name))
        lines = {}
    ...
    def _get_data_from_mapping_config(self, order_lines, edi):
        ...
        try:
            ...
            raise ValidationError(
                self.env._("Error in python code mapping values:\n %s", str(e))
            ) from None
        return data
    ...
    def _prepare_data_lines(self, lines, edi):
        ...
        self.ensure_one()
        data = f"""
            {edi.constant_file_start}A{edi.vrp_code}B{edi.customer_code}
            {self._get_data_from_mapping_config(lines, edi)}{edi.constant_file_end}
            """
        return data
    ...
    def _process_send_ftp(self):
        ...
        if not edi_system:
            raise ValidationError(
                self.env._("No Config FTP for this supplier %s!", self.partner_id.name)
            ) from None
        # Prepare data file
        data_lines = self._prepare_data_lines(lines, edi_system)
        # Params
        pattern = edi_system.po_text_file_pattern
        ...
        self.env["purchase.edi.log"].create_log_history(
            self.env._("Orders interface"), edi_system.id
        )
        # Close FTP
    ...
    def button_confirm(self):
        ...
        for order in self.filtered(lambda order: order.partner_id.is_edi):
