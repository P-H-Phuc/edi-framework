...
class InvoiceSupplierPriceUpdate(models.TransientModel):
    ...
    edi_line_ids = fields.One2many(
        comodel_name="supplier.info.update.line", inverse_name="inv_supplier_price_id"
    )
    ...
    def compute_process_lines(self, active_model, active_obj):
        ...
        price_date = (
            active_model == "account.invoice" and active_obj.date_invoice or False
        )
        # END
            ...
            if selected_seller_id:
                ...
                if partner_id.is_edi:
                    ...
                    edi_price = supplier_price_list_obj.search(
                        price_domain, order="apply_date desc"
                    )
                    if edi_price:
    ...
    def update_prices_second(self):
        ...
        for line in lines:
            ...
            if active_model == "account.invoice":
                obj_lines_values.append(
                    (1, line.invoice_line_id.id, {"price_unit": updated_price_unit})
                )
            elif active_model == "purchase.order":
                obj_lines_values.append(
                    (1, line.po_line_id.id, {"price_unit": updated_price_unit})
                )
        if active_model == "account.invoice":
